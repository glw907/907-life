<!-- @component 907's home: a masthead over a composed front page, the newest post given a lead
...
<svelte:head>
  ...
  <meta name="description" content="A personal blog by Geoffrey L. Wright." />
</svelte:head>

<section class="mx-auto max-w-measure pb-xl pt-l">
  <h1 class="m-0 mb-s font-display text-step-5 font-semibold leading-tight tracking-tight">907.life</h1>
  <p class="m-0 max-w-[38rem] text-step-1 leading-snug text-muted">
    A personal blog by Geoffrey L. Wright, written from Alaska.
  </p>
</section>
  ...
  <div class="index">
    ...
    {#if entries.length > TAG_FILTER_MIN_ENTRIES && tagOptions.length > 0}
      <div class="tag-filter" role="group" aria-label="Filter by tag">
        <button type="button" class="tag-filter__option" aria-pressed={selected === ''} onclick={() => (selected = '')}>
          All
        </button>
