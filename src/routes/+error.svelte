<!-- @component The site's ROOT error page, a sibling of the `(site)` route group rather than a
     ...
     every leaf besides `/about`, which opts out for its own contact-form reasons), so SvelteKit
     ...
     `SiteFooter`, the same components and token classes the `(site)` layout uses. See
...
<svelte:head>
  ...
  <title>{page.status} · 907.life</title>
</svelte:head>
